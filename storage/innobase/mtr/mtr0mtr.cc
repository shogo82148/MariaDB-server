...
uint32_t mtr_t::page_checksum(byte *page)
{
  ut_a(page);
  lsn_t lsn = mach_read_from_8(page + FIL_PAGE_LSN);
  mach_write_to_8(page + FIL_PAGE_LSN, 0);
  uint32_t checksum = buf_calc_page_crc32(page);
...
struct WriteChecksum
{
  ...
  bool operator()(const mtr_memo_slot_t *slot)
  {
    if (slot->type & MTR_MEMO_MODIFY)
    {
      ...
      {
        uint32_t crc= mtr_t::page_checksum(block->frame);
        DBUG_EXECUTE_IF("ib_log_corrupt_option_crc", crc+= 10;);
        m_mtr.page_checksum(page_id, crc,
