  ...
  apply_status apply(const buf_block_t &block, uint16_t &last_offset) const
  {
    ...
    for (const byte *l= recs;;)
    {
      ...
      case OPTION:
        ...
        calculated_checksum= mtr_t::page_checksum(frame);
        if (UNIV_UNLIKELY(checksum != calculated_checksum))
          ib::warn() << "Page checksum stored in redo log record " << checksum
                     << " does not match counted checksum "
                     << calculated_checksum << " for page " << block.page.id()
                     << ".";
        continue;
      case EXTENDED:
